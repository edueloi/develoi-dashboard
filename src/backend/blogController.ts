...
export const blogController = {
  ...
  createCategory: async (req: Request, res: Response) => {
    try {
      const { name, description, color = "#6366f1", isActive = true, sortOrder = 0 } = req.body;
  ...
  createAuthor: async (req: Request, res: Response) => {
    try {
      const { name, bio, photo, role, linkedin, isActive = true } = req.body;
      if (!name) return res.status(400).json({ error: "Nome é obrigatório" });
      let slug = toSlug(name);
      const ex = await (prisma as any).blogAuthor.findFirst({ where: { slug } });
      if (ex) slug = `${slug}-${Date.now()}`;
      const author = await (prisma as any).blogAuthor.create({
        data: { id: randomUUID(), name, slug, bio, photo, role, linkedin, isActive },
      });
      res.json(author);
    } catch (e: any) {
      res.status(500).json({ error: e.message });
  ...
  updateAuthor: async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const { name, bio, photo, role, linkedin, isActive } = req.body;
      const data: any = {};
      if (name !== undefined) { data.name = name; data.slug = toSlug(name); }
  ...
  getStats: async (_req: Request, res: Response) => {
    try {
      ...
      res.json({
        ...
        totalSubscribers, activeSubscribers, totalCategories, totalAuthors, topPosts,
      });
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
